...
(`/cards`) — all with the honesty pattern: explicit `building`/`truncated`/invite states, null
team renders as absent, omitted fields render "not returned", unknown is never zero. Client now
declares 59, reaches 57 of 59. `/me/*` writes (key create/revoke/rotate, team invite/seats,
billing) and `memory/cards/{dream,edit,revert}` remain unwired — mutations are their own commits.
