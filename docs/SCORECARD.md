...
| Can the public repository pass its release validation from a clean checkout? | GitHub Actions run `31911232894`, tag `v0.2.4`, Ubuntu clean runner, 2026-08-15 | 1 completed clean run | Tag/version, installer, archive reproducibility, and fork provenance passed before locked Rust tests | REFUSED: 3,304 TUI library tests passed before `rust_brief_decision_matches_the_python_contract` exposed a parent-repository dependency | The standalone repair recorded with this scorecard, platform builds, release creation, npm publication, or artifact readback |
