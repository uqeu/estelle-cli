...
|---|---|---|
| Billing/catalog settings | `GET /settings` and `POST /settings` read the catalog or write one `{key, value}` (`src/estelle/serve/api.py:911-916`). | Bound. `BillingCatalog` declares GET + POST; `/billing` reads the catalog and `/billing set <key>=<value>` writes exactly one server setting (`cli-rs/estelle-client/src/endpoint.rs`, `cli-rs/tui/src/commands.rs`). Malformed assignments produce no request. This is a command surface, not yet an interactive catalog picker; writes remain admin/owner-gated by the server. |
...
| Sweep client fence receipt | `/sync` and `/ingest/start` accept JSON objects but currently neither preserve nor echo a client-fence receipt. | Current clients attach `client_fence: {gitignore, secret_scan}` to both sweep transports. `gitignore` is `exclude-standard` only after a successful Git inventory and `not-applicable` for a plain directory. The server half remains required: preserve and echo this object as an upload receipt; absence means `not reported`, never `unfenced` or `fenced`. Until that lands, request capture proves the client sent the claim but no production response proves the server observed it. |
