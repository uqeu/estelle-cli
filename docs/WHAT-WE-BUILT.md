...
| Provider-as-data catalogue with canonical ids, aliases, auth kinds, surface membership, endpoint defaults, and key requirements | jcode `crates/jcode-provider-metadata/src/catalog.rs`, `jcode-provider-core` · MIT | `tui/src/provider_catalog.rs` | 🟡 |
| GitHub Copilot device-code acquisition with bounded polling, private token snapshot, and secret-free failure/receipt paths | jcode `crates/jcode-base/src/auth/copilot.rs:40`, `:727` · MIT | `tui/src/copilot_login.rs`, `tui/src/provider_store.rs` | 🟡 |
| Local/custom endpoint acquisition with client-side private metadata and no-key local-network policy | jcode LM Studio/Ollama profiles and Goose local-inference registry boundary · MIT | `tui/src/local_provider.rs`, `tui/src/provider_catalog.rs`, `tui/src/provider_store.rs` | 🟡 |
