...
| Working memory | `tui/src/top_level.rs:706` collects changed, staged and untracked Git source files. `tui/src/main.rs` `answer_question` attaches them as model input on the single `/deep-search` call for non-conversational questions; conversational turns skip the upload so the server's fast path can fire (D1, 2026-08-07). | Type any ordinary question while the repo has eligible local changes. There is no picker and no second identity. | **Yes**, automatic on the real question path. | Ported because no server owner can represent one developer's uncommitted files. |
