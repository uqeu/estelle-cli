...
| S7 | Server contract doors the client can't open (safe defaults today): ~~`files` on /scan~~ — **WIRED** (lockfiles the diff touches); ~~`messages` on /skill/run~~ — **WIRED** (per-skill threads continue the conversation); ~~`partial` on /sync~~ — **VERIFIED NOTHING TO WIRE**: one call site, always the complete inventory, so absent-`partial` is correct and no subset flow exists; `cwd` on /deep-search — **HELD FOR SERVER COORDINATION** (their S3 makes CWD beat scope_ask; sending it before that lands inverts the intended precedence); `tokens`/`needs_reasoning` on /route, `surface` + POST on /autonomy/scope (register #72's remainder). ~~`deep:true` on /gate~~ — **WIRED**: `/review` now sends `deep: true` (was wire-identical to `/gate`); the deep-changed-outcome disclosure renders from the server's `deterministic` key. | per-route evidence in the group audits | ⚪ recorded as capability gaps, none faked |
