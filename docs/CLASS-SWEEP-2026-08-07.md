...
| S7 | Server contract doors the client can't open (safe defaults today): `files` on /scan (lockfile CVE scan), `messages` on /skill/run (interactive skills restart single-turn), `partial` on /sync, `cwd` on /deep-search, `tokens`/`needs_reasoning` on /route, `surface` + POST on /autonomy/scope (register #72's remainder). ~~`deep:true` on /gate~~ — **WIRED**: `/review` now sends `deep: true` (was wire-identical to `/gate`); the deep-changed-outcome disclosure renders from the server's `deterministic` key. | per-route evidence in the group audits | ⚪ recorded as capability gaps, none faked |
