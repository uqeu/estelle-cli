...
const ESTELLE_AUTH_SECRETS_FILENAME: &str = "estelle_auth.age";
...
pub enum LocalSecretsNamespace {
    ...
    CodexAuth,
    /// Estelle API credentials used by the Estelle CLI and TUI.
    EstelleAuth,
...
impl LocalSecretsBackend {
    ...
    fn secrets_path(&self) -> PathBuf {
        let filename = match self.namespace {
            ...
            LocalSecretsNamespace::EstelleAuth => ESTELLE_AUTH_SECRETS_FILENAME,
    ...
    fn save_file(&self, file: &SecretsFile) -> Result<()> {
        ...
        set_mode(&dir, 0o700)
            .with_context(|| format!("failed to secure secrets dir {}", dir.display()))?;
...
fn write_file_atomically(path: &Path, contents: &[u8]) -> Result<()> {
    ...
    match fs::rename(&tmp_path, path) {
        Ok(()) => {
            set_mode(path, 0o600)
                .with_context(|| format!("failed to secure secrets file {}", path.display()))?;
            Ok(())
        }
        Err(initial_error) => {
...
#[cfg(unix)]
fn set_create_mode(options: &mut fs::OpenOptions, mode: u32) {
    use std::os::unix::fs::OpenOptionsExt;
    options.mode(mode);
}

#[cfg(not(unix))]
fn set_create_mode(_options: &mut fs::OpenOptions, _mode: u32) {}
...
mod tests {
    ...
    #[cfg(unix)]
    #[test]
    fn estelle_auth_namespace_uses_estelle_keyring_and_mode_0600() -> Result<()> {
        use std::os::unix::fs::PermissionsExt;

        let estelle_home = tempfile::tempdir().expect("tempdir");
        let keyring = Arc::new(MockKeyringStore::default());
        let backend = LocalSecretsBackend::new_with_namespace(
            estelle_home.path().to_path_buf(),
            keyring,
            LocalSecretsNamespace::EstelleAuth,
        );
        let scope = SecretScope::Global;
        let name = SecretName::new("ESTELLE_API_KEY")?;

        backend.set(&scope, &name, "estelle-key")?;

        let path = estelle_home.path().join("secrets").join("estelle_auth.age");
        assert!(path.exists());
        assert_eq!(fs::metadata(path)?.permissions().mode() & 0o777, 0o600);
        assert_eq!(
            keyring_service(LocalSecretsNamespace::EstelleAuth),
            "estelle"
        );
        Ok(())
    }
