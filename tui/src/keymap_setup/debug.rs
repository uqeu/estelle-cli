...
const SHORT_MISSING_KEY_HINT: &str = "Estelle can only inspect keys the terminal sends.";
const DELAYED_MISSING_KEY_HINT: &str = "If nothing changes on a keypress, the terminal is not sending that key. Only received keys can be bound.";
...
impl KeymapDebugView {
    ...
    fn lines_at(&self, width: u16, now: Instant) -> Vec<Line<'static>> {
        ...
        let mut lines = vec![
            ...
            Line::from(
                "Press any key to see what Estelle receives. Esc is inspected; Ctrl+C closes."
                    .dim(),
            ),
        ];
